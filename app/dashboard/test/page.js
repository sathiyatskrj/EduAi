...
export default function TestGenerator() {
  ...
  const generate = async () => {
    ...
    incrementAiUsage();
    try {
      const systemPrompt = `You are EduAI, an expert question paper generator for Indian CBSE/State Board exams. Generate complete, print-ready question papers with: school header, general instructions, sections (A/B/C), question numbers with marks in brackets, answer key with solutions, marking scheme, and a blueprint table showing topic vs Bloom's taxonomy distribution.\n\nIMPORTANT: Do NOT use LaTeX or dollar-sign math notation ($, $$, \\frac, \\sqrt, etc.). Write all mathematical expressions in plain readable text using standard symbols: × for multiplication, ÷ for division, ² ³ for exponents (e.g., x² + 3x + 5 = 0), √ for square root, π for pi, ≤ ≥ ≠ for comparisons, fractions as (a/b), and so on. The output must be readable without any math rendering engine.`;
